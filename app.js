...
const handleQuestion = () => {
  ...
  let optionPrint = quizQuestions[indexNumber].options;
  question.innerHTML = questionPrint;
  optionsList.innerHTML = "";
  for (let key in optionPrint) {
    optionsList.innerHTML += `
    <form>
    <label>
...
const submission = () => {
  ...
  let per = (correctAns / quizQuestions.length) * 100;
...
const checkAnswer = (element) => {
  ...
  // let optionClick = element
  console.log("allOptions", allOptions);
  // return;
  //   let optionClick = element.innerText || element.textContent;
  let optionClick = element.value;
  let answerOption = quizQuestions[indexNumber].answer;
